...
<div className={getContainerClasses({ fullWidth, removePaddingLeft, removePaddingRight })}>
            ...
            <div className="grid">
                <div className="col-mobile-4 col-tablet-8 col-desktop-12">
                    ...
                    <video src={image} controls autoPlay muted loop playsInline className={`${styles.image} imagePlaceholder-5-4 object-cover`} id={(imageId || (typeof sectionId !== 'undefined' && sectionId ? `${sectionId}-image` : undefined))} />
                ) : (image && /\.(mp3|wav)(\?.*)?$/i.test(image)) ? (
                    <audio src={image} controls className={`${styles.image} imagePlaceholder-5-4 object-cover`} id={(imageId || (typeof sectionId !== 'undefined' && sectionId ? `${sectionId}-image` : undefined))} />
                ) : (
                    <img src={image || null} alt="" className={`${styles.image} imagePlaceholder-5-4 object-cover`} id={(imageId || (typeof sectionId !== 'undefined' && sectionId ? `${sectionId}-image` : undefined))} />
                ))}
                    </div>
                </div>
            </div>
