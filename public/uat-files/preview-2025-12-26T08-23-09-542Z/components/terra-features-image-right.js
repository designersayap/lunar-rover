            ...
            <div className={getContainerClasses({ fullWidth, removePaddingLeft, removePaddingRight })}>
                <div className="grid align-center">
                    <div className={`${styles.content} col-mobile-4 col-tablet-8 col-desktop-6`}>
                        ...
                        <p className={`subheader-h2 ${styles.description}`} id={(typeof sectionId !== 'undefined' && sectionId ? `${sectionId}-p` : undefined)} style={{ color: "var(--content-neutral--caption)", marginBottom: "var(--gap-md)" }}>{subtitle}</p>
                        <div className="buttonWrapperLeft">
                            <Link href={(
                   (buttonLinkType === 'dialog' && buttonTargetDialogId)
                     ? '#' + buttonTargetDialogId
                     : (buttonUrl || "#")
                )} className={`btn btn-${buttonStyle || 'primary'} btn-md`} id={(buttonId || (typeof sectionId !== 'undefined' && sectionId ? `${sectionId}-button` : undefined))} onClick={(e) => { if (buttonLinkType === 'dialog' && buttonTargetDialogId) { window.location.hash = '#' + buttonTargetDialogId; } }}>{buttonText}</Link>
                            <Link href={(
